...
```kotlin
implementation("com.debugbundle:debugbundle-spring-boot-starter:1.4.0")
```
...
```text
-javaagent:/opt/debugbundle/debugbundle-java-agent-1.4.0.jar=config=/etc/debugbundle/debugbundle.properties,capture-jul=true,capture-uncaught=true
```
...
```kotlin
dependencies {
  implementation(platform("com.debugbundle:debugbundle-java-parent:1.4.0"))
...
The agent is a bootstrap path, not a bytecode-instrumentation product. Its
published JAR is self-contained, initializes the singleton SDK early, and
installs the existing uncaught-exception and `java.util.logging` hooks.
